function make_figure(figName, config = {}) {
...
if (!('showTFNRPD' in config)) {
    config['showTFNRPD'] = true;
}
if (!('showHoF' in config)) {
    config['showHoF'] = true;
}
...
let b1 = config['b1'];
    b1.x = b1.x.map(x => x*width);
    b1.y = b1.y.map(y => y*height);
let b2 = config['b2'];
    b2.x = b2.x.map(x => x*width);
    b2.y = b2.y.map(y => y*height);
...
let box_plots_svg = d3.select(`div#${figName}`).append('svg')
        .attr("width", half_width)
        .attr("height", 2*mf_plot_outer_height + mf_margin_bottom)
        .style("margin-bottom", mf_margin_bottom+'px')
        .style("float", "right");
if (!config['showTFNRPD']) { box_plots_svg.style('display', 'none'); }
let box_plots = box_plots_svg.append("g")
...
let hof_plots_0_svg = d3.select(`div#${figName}`).append('svg')
   ...
   .attr("height", mf_plot_outer_height)
   .style("margin-bottom", mf_margin_bottom+'px');
if (!config['showHoF']) { hof_plots_0_svg.style('display', 'none'); }
...
let hof_plots_2_svg = d3.select(`div#${figName}`).append('svg')
   ...
   .attr("height", mf_plot_outer_height)
   .style("margin-bottom", mf_margin_bottom+'px');
if (!config['showHoF']) { hof_plots_2_svg.style('display', 'none'); }
...
let plots_svg = d3.select(`div#${figName}`).append('svg')
              .attr("width", width)
              .attr("height", plot_outer_height)
              .style("margin-bottom", mf_margin_bottom+'px');

if (!config['showMeasures']) {
    plots_svg.style('display', 'none');
}

let plots = plots_svg.append("g")
...
function computeStats() {
    ...
    try {
        ...
        let mf_data, box_data;
        if (config['showPair2']) {
            mf_data = [d1, d2];
            box_data = [b1, b2, b3, b4];
        } else {
            mf_data = [d1];
            box_data = [b1, b2];
        }
...
function makeDraggableBox(box, show=true) {
...
if (config['showPair2']) {
    ...
    makeDraggableBox(b3, false);
    makeDraggableBox(b4, false);
}
